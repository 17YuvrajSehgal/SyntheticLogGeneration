...
def std_normal(size):
    ...
    """
    # Use device from size if it's a tensor, otherwise default to cuda if available, else cpu
...
def calc_diffusion_step_embedding(diffusion_steps, diffusion_step_embed_dim_in):
    ...
    # Use device from diffusion_steps
    ...
    _embed = torch.exp(torch.arange(half_dim, device=device) * -_embed).to(device)
