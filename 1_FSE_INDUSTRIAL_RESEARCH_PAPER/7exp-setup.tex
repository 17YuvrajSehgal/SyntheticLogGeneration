...
This section describes the compute environment, datasets, experimental design, and sampling procedure used to evaluate the proposed diffusion-based kernel trace generator. All experiments use a unified training and generation pipeline and differ only in controlled variables such as temporal context length and feature availability. Figure~\ref{fig:downstream-training} illustrates our evaluation methodology.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{figures/2bg.png}
\caption{Downstream training evaluation methodology. We conduct independent training runs using: (1) Real-Only: baseline trained on 50\% real data, (2) Synthetic-Only: trained on synthetic data (raw or repaired), and (3) Combined: trained on 50\% real + 50\% synthetic data. All configurations are evaluated on the same held-out real test set using a Transformer-based next-event predictor. This setup measures how well synthetic data captures real system behavior and whether it can effectively augment limited real data.}
\label{fig:downstream-training}
\end{figure}
