...
\paragraph{Canonical Event Fields.}
Although individual kernel events expose rich and heterogeneous metadata, directly modeling all fields would be impractical and unnecessary for our objectives. Instead, we extract a canonical set of attributes that are consistently available across events and sufficient to capture execution dynamics:
(i) the event type (e.g., \texttt{kmem\_kmalloc}, \texttt{sched\_wakeup}),
(ii) the inter-event time difference, and
(iii) the CPU identifier.
These attributes form the basis of our downstream representations and enable scalable modeling across benchmarks and runs.

At this stage, the output of preprocessing is a cleaned, time-ordered textual representation of kernel events, serving as an intermediate format between raw LTTng traces and the machine-learning-ready data structures described next.
...
Each unique event name is assigned a deterministic integer identifier. Event identifiers are ordered by decreasing frequency, such that more common events receive smaller identifiers. This ordering is fixed and reused throughout all experiments, ensuring that models trained on different subsets of the data share an identical semantic encoding of events. In addition to the forward mapping (\texttt{event\_name} $\rightarrow$ \texttt{event\_id}), we retain the inverse mapping for interpretability and post-hoc analysis.

Beyond event names, we construct auxiliary vocabularies for selected high-frequency payload attributes that exhibit categorical structure. In particular, we build vocabularies for process command names (\texttt{comm}) and return values (\texttt{ret}). The \texttt{comm} vocabulary is kept in full due to its limited cardinality, while the \texttt{ret} vocabulary is truncated to the top-$K$ most frequent values to control long-tail sparsity. All vocabularies reserve dedicated tokens for padding and unknown values, and are frozen once constructed to prevent drift across experiments.

\paragraph{Enriched Parquet Representation.}
To support scalable analysis and efficient training, we convert the decoded text traces into a columnar, organized representation using Apache Parquet. Each trace run is processed independently, resulting in one Parquet file per benchmark run. This design enables embarrassingly parallel preprocessing and avoids cross-run coupling during storage.
...
For each Parquet trace, we extract ordered sequences of event-level attributes and apply feature-specific transformations. Event identifiers and CPU identifiers are preserved as discrete categorical values. Inter-event time deltas are log-normalized using $\log(1 + \Delta t)$ to reduce scale variance while preserving temporal ordering. Thread identifiers are hashed into a fixed number of buckets to retain local thread identity without introducing unbounded cardinality. File descriptors are clamped to a maximum value to control sparsity. Categorical fields such as process command names and return values are mapped using the frozen vocabularies described earlier.

We generate fixed-length windows of length $L$ using a sliding window with fixed stride, producing multiple overlapping sequences from a single execution trace. Each window is stored as a set of aligned tensors, including event identifiers, time deltas, CPU identifiers, thread identifiers, file descriptors, command identifiers, and return-value identifiers. All tensors share the same shape $(N, L)$, where $N$ denotes the number of windows extracted from a trace.

To support experimentation with varying temporal contexts, we materialize multiple windowed datasets with different sequence lengths. This design enables systematic evaluation of short-, medium-, and long-context models without modifying upstream preprocessing logic. The resulting datasets are stored as compressed NumPy archives (\texttt{.npz}), which provide fast loading, low storage overhead, and seamless integration with common machine learning frameworks.
