"""
Training script using official SSSD repository for kernel trace generation.
Adapted to new trace data format:
- data_sequences/compress-gzip-all-events-run*.npy
- each .npy has shape (N, 3): [event_id, dt, cpu_id]
We currently train SSSD on the event_id channel only.
"""

import os
import sys
import argparse
import json
import numpy as np
import torch
import torch.nn as nn
from pathlib import Path

# Add project root to path for our modules
project_root = os.path.join(os.path.dirname(__file__), '..')
project_root = os.path.abspath(project_root)
if project_root not in sys.path:
    sys.path.insert(0, project_root)

# NOTE: we no longer use TraceDataset/DataLoader; we load run*.npy directly.

# Add SSSD src to path BEFORE importing
sssd_src_path = os.path.join(os.path.dirname(__file__), '..', 'SSSD', 'src')
sssd_src_path = os.path.abspath(sssd_src_path)

if not os.path.exists(sssd_src_path):
    raise FileNotFoundError(
        f"SSSD src directory not found at: {sssd_src_path}\n"
        f"Please ensure the SSSD repository is cloned in the SSSD/ directory."
    )

# Add to sys.path
if sssd_src_path not in sys.path:
    sys.path.insert(0, sssd_src_path)

# Import SSSD modules - they use relative imports from src directory
original_cwd = os.getcwd()

if sssd_src_path in sys.path:
    sys.path.remove(sssd_src_path)
sys.path.insert(0, sssd_src_path)

os.chdir(sssd_src_path)
try:
    modules_to_remove = [
        k for k in sys.modules.keys()
        if k.startswith('imputers.') or k.startswith('utils.')
    ]
    for mod in modules_to_remove:
        del sys.modules[mod]

    from imputers.SSSDS4Imputer import SSSDS4Imputer
    from utils.util import (
        calc_diffusion_hyperparams,
        print_size,
        find_max_epoch,
        get_mask_rm,
        get_mask_bm,
        get_mask_mnr,
        training_loss,
    )
finally:
    os.chdir(original_cwd)


# ---------------------------------------------------------------------
# NEW: data loader for run*.npy files
# ---------------------------------------------------------------------
def load_trace_data(runs_dir, sequence_length=200, normalize=True):
    """
    Load trace data from a directory of run .npy files and convert
    to the format expected by SSSD.

    runs_dir:
        Directory containing *.npy files, each of shape (N, 3):
        [event_id, dt, cpu_id].

    Returns:
        data:     (num_sequences, 1, sequence_length) float32
                  (only event_id channel, optionally normalized)
        data_min: float (min event_id before normalization)
        data_max: float (max event_id before normalization)
    """
    runs_dir = Path(runs_dir)
    if not runs_dir.exists():
        raise FileNotFoundError(f"Runs directory not found: {runs_dir}")

    npy_files = sorted(runs_dir.glob("*.npy"))
    if not npy_files:
        raise FileNotFoundError(f"No .npy files found in {runs_dir}")

    print(f"Loading runs from {runs_dir} ({len(npy_files)} files)")

    sequences = []
    all_values = []

    for path in npy_files:
        arr = np.load(path)
        if arr.ndim != 2 or arr.shape[1] < 1:
            print(f"  WARNING: unexpected shape for {path}: {arr.shape}, skipping")
            continue

        # Column 0 is event_id
        event_ids = arr[:, 0].astype(np.float32)
        all_values.append(event_ids)

        total = len(event_ids)
        if total < sequence_length:
            # Skip very short runs
            continue

        # Non-overlapping chunks of length sequence_length
        for start in range(0, total - sequence_length + 1, sequence_length):
            seq = event_ids[start:start + sequence_length]
            sequences.append(seq)

    if not sequences:
        raise RuntimeError(
            f"No sequences extracted from {runs_dir} with sequence_length={sequence_length}"
        )

    data = np.stack(sequences, axis=0)  # (num_sequences, sequence_length)
    all_values = np.concatenate(all_values)
    data_min = float(all_values.min())
    data_max = float(all_values.max())

    if normalize:
        denom = (data_max - data_min) if data_max > data_min else 1.0
        data = (data - data_min) / denom

    # SSSD expects (num_samples, channels, seq_len)
    data = data[:, np.newaxis, :]  # (num_sequences, 1, sequence_length)
    return data, data_min, data_max


# ---------------------------------------------------------------------
# Training
# ---------------------------------------------------------------------
def train(
    output_directory,
    train_runs_dir,
    test_runs_dir,
    ckpt_iter,
    n_iters,
    iters_per_ckpt,
    iters_per_logging,
    learning_rate,
    sequence_length=200,
    batch_size=32,
    masking='rm',
    missing_k=10,
):
    """
    Train SSSDS4 model on kernel trace data.

    Parameters:
    output_directory (str):   save model checkpoints to this path
    train_runs_dir (str):     directory with training run .npy files
    test_runs_dir (str):      directory with test run .npy files
    ckpt_iter (int or 'max'): checkpoint to resume from
    n_iters (int):            number of training iterations
    iters_per_ckpt (int):     save checkpoint every N iterations
    iters_per_logging (int):  log every N iterations
    learning_rate (float):    learning rate
    sequence_length (int):    length of sequences
    batch_size (int):         batch size
    masking (str):            'mnr', 'bm', or 'rm'
    missing_k (int):          number of missing points
    """

    # Diffusion hyperparameters
    T = 200
    beta_0 = 0.0001
    beta_T = 0.02

    diffusion_config = {"T": T, "beta_0": beta_0, "beta_T": beta_T}
    diffusion_hyperparams = calc_diffusion_hyperparams(T, beta_0, beta_T)

    # Model configuration for trace data (1 channel, sequence length)
    model_config = {
        "in_channels": 1,
        "out_channels": 1,
        "num_res_layers": 24,  # Reduced from 36 for faster training
        "res_channels": 128,   # Reduced from 256
        "skip_channels": 128,
        "diffusion_step_embed_dim_in": 128,
        "diffusion_step_embed_dim_mid": 512,
        "diffusion_step_embed_dim_out": 512,
        "s4_lmax": sequence_length,
        "s4_d_state": 64,
        "s4_dropout": 0.0,
        "s4_bidirectional": 1,
        "s4_layernorm": 1,
    }

    # Generate experiment path
    local_path = f"T{T}_beta0{beta_0}_betaT{beta_T}"
    output_directory = os.path.join(output_directory, local_path)
    os.makedirs(output_directory, exist_ok=True)
    print("Output directory:", output_directory, flush=True)

    # Map diffusion hyperparameters to device
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    for key in diffusion_hyperparams:
        if key != "T":
            diffusion_hyperparams[key] = diffusion_hyperparams[key].to(device)

    # Load data from runs
    print("Loading training data from runs...")
    train_data, train_min, train_max = load_trace_data(
        train_runs_dir, sequence_length, normalize=True
    )
    print(f"Training data shape: {train_data.shape}")
    print(f"Training event_id range: [{train_min}, {train_max}]")

    print("Loading test data from runs...")
    test_data, test_min, test_max = load_trace_data(
        test_runs_dir, sequence_length, normalize=True
    )
    print(f"Test data shape: {test_data.shape}")
    print(f"Test event_id range: [{test_min}, {test_max}]")

    # Save normalization info
    norm_info = {
        'train_min': float(train_min),
        'train_max': float(train_max),
        'test_min': float(test_min),
        'test_max': float(test_max),
    }
    with open(os.path.join(output_directory, 'normalization.json'), 'w') as f:
        json.dump(norm_info, f, indent=2)

    # Convert to torch tensors
    train_data = torch.from_numpy(train_data).float().to(device)
    test_data = torch.from_numpy(test_data).float().to(device)

    # Create model
    print("Creating model...")
    net = SSSDS4Imputer(**model_config).to(device)
    print_size(net)

    # Optimizer
    optimizer = torch.optim.Adam(net.parameters(), lr=learning_rate)

    # Load checkpoint if resuming
    if ckpt_iter == 'max':
        ckpt_iter = find_max_epoch(output_directory)
    if ckpt_iter >= 0:
        try:
            model_path = os.path.join(output_directory, f'{ckpt_iter}.pkl')
            checkpoint = torch.load(model_path, map_location=device)
            net.load_state_dict(checkpoint['model_state_dict'])
            if 'optimizer_state_dict' in checkpoint:
                optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            print(f'Successfully loaded model at iteration {ckpt_iter}')
        except Exception as e:
            print(f'Failed to load checkpoint: {e}')
            ckpt_iter = -1
            print('No valid checkpoint found, starting from scratch.')
    else:
        ckpt_iter = -1

    # Training loop
    print("Starting training...")
    print(f"Total iterations: {n_iters}, Starting from: {ckpt_iter + 1}")
    net.train()

    from tqdm import tqdm
    pbar = tqdm(range(ckpt_iter + 1, n_iters), desc="Training", unit="iter")

    for n in pbar:
        # Sample random batch
        batch_indices = np.random.choice(len(train_data), size=batch_size, replace=False)
        batch = train_data[batch_indices]  # (batch_size, 1, seq_len)

        # Build masks
        masks = []
        for i in range(batch_size):
            # sample: (1, seq_len) -> (seq_len, 1)
            sample = batch[i, :, :].cpu().permute(1, 0)

            if masking == 'rm':
                sample_mask = get_mask_rm(sample, missing_k)
            elif masking == 'bm':
                sample_mask = get_mask_bm(sample, missing_k)
            elif masking == 'mnr':
                sample_mask = get_mask_mnr(sample, missing_k)
            else:
                raise ValueError(f"Unknown masking strategy: {masking}")

            if isinstance(sample_mask, np.ndarray):
                sample_mask = torch.from_numpy(sample_mask).float()
            elif not isinstance(sample_mask, torch.Tensor):
                sample_mask = torch.tensor(sample_mask, dtype=torch.float32)

            sample_mask = sample_mask.permute(1, 0)  # (1, seq_len)
            masks.append(sample_mask)

        mask = torch.stack(masks, dim=0).to(device)  # (batch_size, 1, seq_len)

        loss_mask = ~mask.bool()
        X = (batch, batch, mask, loss_mask)
        loss = training_loss(net, nn.MSELoss(), X, diffusion_hyperparams, only_generate_missing=1)

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        pbar.set_postfix({'loss': f'{loss.item():.4f}'})

        # Logging + validation
        if n % iters_per_logging == 0:
            print("\niteration: {} \tloss: {:.6f}".format(n, loss.item()), flush=True)

            net.eval()
            with torch.no_grad():
                val_indices = np.random.choice(
                    len(test_data),
                    size=min(batch_size, len(test_data)),
                    replace=False,
                )
                val_batch = test_data[val_indices]

                val_masks = []
                for i in range(len(val_batch)):
                    sample = val_batch[i, :, :].cpu().permute(1, 0)
                    val_sample_mask = get_mask_rm(sample, missing_k)

                    if isinstance(val_sample_mask, np.ndarray):
                        val_sample_mask = torch.from_numpy(val_sample_mask).float()
                    elif not isinstance(val_sample_mask, torch.Tensor):
                        val_sample_mask = torch.tensor(val_sample_mask, dtype=torch.float32)

                    val_sample_mask = val_sample_mask.permute(1, 0)
                    val_masks.append(val_sample_mask)

                val_mask = torch.stack(val_masks, dim=0).to(device)
                val_loss_mask = ~val_mask.bool()
                val_X = (val_batch, val_batch, val_mask, val_loss_mask)
                val_loss = training_loss(
                    net, nn.MSELoss(), val_X, diffusion_hyperparams, only_generate_missing=1
                )
                print("validation loss: {:.6f}".format(val_loss.item()), flush=True)
            net.train()

        # Save checkpoint
        if n % iters_per_ckpt == 0:
            checkpoint = {
                'model_state_dict': net.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'iteration': n,
                'model_config': model_config,
                'diffusion_config': diffusion_config,
                'normalization': norm_info,
            }
            checkpoint_path = os.path.join(output_directory, f'{n}.pkl')
            torch.save(checkpoint, checkpoint_path)
            print('\n✓ Checkpoint saved at iteration {}'.format(n), flush=True)

    print("\n" + "=" * 50)
    print("Training completed!")
    print(f"Total iterations: {n_iters}")
    print(f"Final checkpoint saved at: {output_directory}")
    print("=" * 50)


# ---------------------------------------------------------------------
# CLI
# ---------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='Train SSSDS4 on kernel trace data (event_id channel)')
    parser.add_argument('--dataset', type=str, default='compress-gzip',
                        help='Dataset name (used only for output folder naming)')
    parser.add_argument('--sequence_length', type=int, default=200, help='Sequence length')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size')
    parser.add_argument('--n_iters', type=int, default=10000, help='Number of iterations')
    parser.add_argument('--iters_per_ckpt', type=int, default=1000, help='Save checkpoint every N iterations')
    parser.add_argument('--iters_per_logging', type=int, default=100, help='Log every N iterations')
    parser.add_argument('--lr', type=float, default=2e-4, help='Learning rate')
    parser.add_argument('--masking', type=str, default='rm', choices=['rm', 'bm', 'mnr'],
                        help='Masking strategy')
    parser.add_argument('--missing_k', type=int, default=10, help='Number of missing points')
    parser.add_argument('--output_dir', type=str, default='checkpoints_sssd',
                        help='Base output directory')
    parser.add_argument('--resume', type=str, default='max',
                        help='Resume from checkpoint (iteration number or "max")')

    # NEW: explicit run dirs
    parser.add_argument('--train_runs_dir', type=str, required=True,
                        help='Directory containing training run .npy files')
    parser.add_argument('--test_runs_dir', type=str, required=True,
                        help='Directory containing test run .npy files')

    args = parser.parse_args()

    output_dir = os.path.join(args.output_dir, args.dataset)

    train(
        output_directory=output_dir,
        train_runs_dir=args.train_runs_dir,
        test_runs_dir=args.test_runs_dir,
        ckpt_iter=args.resume,
        n_iters=args.n_iters,
        iters_per_ckpt=args.iters_per_ckpt,
        iters_per_logging=args.iters_per_logging,
        learning_rate=args.lr,
        sequence_length=args.sequence_length,
        batch_size=args.batch_size,
        masking=args.masking,
        missing_k=args.missing_k,
    )


if __name__ == "__main__":
    main()
